...
def remove_foreground(
    input_image_paths, foreground_mask_paths, output_paths, img_res=512, dilation=3
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    if len(input_image_paths) != len(foreground_mask_paths) or len(
        input_image_paths
    ) != len(output_paths):
        raise ValueError(
            "Length of input_image_paths, foreground_mask_paths, and output_paths should be the same."
        )

    inpainter = LamaInpainter()
    inpainter.to(device)

    for input_image_path, foreground_mask_path, output_path in zip(
        input_image_paths, foreground_mask_paths, output_paths
    ):
        ...
        if dilation >= 0:
            ...
            fg_mask = scipy.ndimage.binary_dilation(fg_mask[0, 0], iterations=dilation)[
                None, None, ...
            ]
...
if __name__ == "__main__":
    ...
    foreground_mask_paths = [
        os.path.join(os.path.dirname(path), "mask.png") for path in input_image_paths
    ]
    output_paths = [
        os.path.join(os.path.dirname(path), "bg.png") for path in input_image_paths
    ]
