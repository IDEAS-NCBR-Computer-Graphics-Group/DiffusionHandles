...
def test_diffusion_handles(
    ...
    cache_input_image_identity: bool = False,
    ...
    diff_handles_config = (
        OmegaConf.load(config_path) if config_path is not None else None
    )
    ...
    for sample_idx, (sample_name, transform_names) in enumerate(dataset_names.items()):
        ...
        input_image_identity_path = join(
            tempfile.gettempdir(),
            "diffhandles",
            splitext(basename(test_set_path))[0],
            sample_name,
            "input_image_identity.npz",
        )
        if cache_input_image_identity and exists(input_image_identity_path):
            ...
            null_text_emb = torch.from_numpy(input_image_identity["null_text_emb"]).to(
                device=device
            )
            init_noise = torch.from_numpy(input_image_identity["init_noise"]).to(
                device=device
            )
            activations = [
                torch.from_numpy(input_image_identity[f"activations{i+1}"]).to(
                    device=device
                )
                for i in range(3)
            ]
            latent_image = torch.from_numpy(input_image_identity["latent_image"]).to(
                device=device
            )
        else:
            # 9.
            ...
            null_text_emb, init_noise, activations, latent_image = (
        ...
        save_image(
            recon_image.clamp(min=0, max=1)[0],
            join(output_dir, sample_name, "recon.png"),
        )
        ...
        for transform_name in transform_names:
            ...
            if transform_name not in transforms:
                print(
                    f"WARNING: Transform {transform_name} not found for image {sample_name}. Skipping."
                )
                continue

            if skip_existing and transform_exists[transform_name]:
                print(
                    f"Skipping transform {transform_name} of image {sample_name}, since its output already exists."
                )
                continue
            ...
            translation = (
                torch.tensor(transform["translation"], dtype=torch.float32)
                if "translation" in transform
                else None
            )
            rot_axis = (
                torch.tensor(transform["rotation_axis"], dtype=torch.float32)
                if "rotation_axis" in transform
                else None
            )
            rot_angle = (
                float(transform["rotation_angle"])
            ...
            save_diffhandles_outputs(
                ...
                edited_disparity=edited_disparity,
...
def preprocess_samples(input_dir, dataset_names):
    ...
    for sample_name in dataset_names.keys():
        ...
        if any(
            not exists(join(input_dir, sample_name, fname)) for fname in required_fnames
        ):
            print(
                f"Skipping sample {sample_name}, since it is missing one of the required input files ({required_fnames})."
            )
            incomplete_samples.append(sample_name)
            continue
        if not exists(join(input_dir, sample_name, "bg_depth.exr")) and not exists(
            join(input_dir, sample_name, "bg.png")
        ):
            foreground_removal_samples.append(sample_name)
        if not exists(join(input_dir, sample_name, "depth.exr")) or not exists(
            join(input_dir, sample_name, "bg_depth.exr")
        ):
    ...
    if len(foreground_removal_samples) > 0:
        ...
        remove_foreground(
            input_image_paths=[
                ...
                for sample_name in foreground_removal_samples
            ],
            foreground_mask_paths=[
                join(input_dir, sample_name, "mask.png")
                for sample_name in foreground_removal_samples
            ],
            output_paths=[
                join(input_dir, sample_name, "bg.png")
    ...
    if len(depth_estimation_samples) > 0:
        ...
        estimate_depth(
            input_image_paths=[
                join(input_dir, sample_name, "input.png")
                for sample_name in depth_estimation_samples
            ]
            + [
                join(input_dir, sample_name, "bg.png")
                for sample_name in depth_estimation_samples
            ...
            output_paths=[
                join(input_dir, sample_name, "depth.exr")
                for sample_name in depth_estimation_samples
            ]
            + [
                join(input_dir, sample_name, "bg_depth.exr")
...
def load_diffhandles_inputs(
    ...
    if len(prompt) == 0:
        ...
        if len(prompt) > 1:
            print(
                f"WARNING: prompt for sample {sample_name} has multiple lines, only using the first line."
            )
        prompt = prompt[0]
    ...
    if img.shape[-2:] != (img_res, img_res):
        print(
            f"WARNING: Resizing and cropping image from {img.shape[-2]}x{img.shape[-1]} to {img_res}x{img_res}."
        )
...
def save_diffhandles_inputs(
    ...
    save_image(
        (bg_depth / bg_depth.max())[0], join(output_dir, sample_name, "bg_depth.png")
    )
...
def save_diffhandles_outputs(
    ...
    save_image(
        (edited_disparity / edited_disparity.max())[0],
        join(output_dir, sample_name, f"{transform_name}_disparity.png"),
    )
    ...
    if denoising_steps is not None:
        denoising_steps_dir = join(
            output_dir, sample_name, f"{transform_name}_denoising_steps"
        )
        ...
        for denoising_idx in range(len(denoising_steps["opt"])):
            for opt_idx in range(len(denoising_steps["opt"][denoising_idx])):
                denoising_steps["opt"][denoising_idx][opt_idx] = denoising_steps["opt"][
                    denoising_idx
                ][opt_idx]
                save_image(
                    denoising_steps["opt"][denoising_idx][opt_idx][0],
                    join(
                        denoising_steps_dir,
                        f"step_{denoising_idx}_opt_{opt_idx}.png",
                    ),
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--test_set_path", type=str, default="data/photogen/photogen.json"
    )
